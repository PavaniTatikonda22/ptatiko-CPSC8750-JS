...
const onClick = event => {
  ...
  if (selected && gameState.moves[selected].includes(square)) {
    ...
    console.log('AI clicked from ' +movedFrom + "to" +movedTo);
    gameState = game.exportJson();
...
function addCursorStyle(square,gameState){
    if (square != null) {
...
function removeCursorStyle(square,gameState){
    if (square != null) {
        if (gameState.moves[square] != null) {
            gameState.moves[square].forEach((element) => {
                const el = document.getElementById(element);
